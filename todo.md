...
- [x] CURR-1: rate_sources registry table — 9 official sources covering all 23 filed rows (URL, label, governed urdbIds, adjustor cycle, cadence, sha256 content fingerprint, fail counts); migration 0019
- [x] CURR-2: tariffs schema — sourceUrl, lastVerifiedAt, verifyStatus columns; seeder stamps filed rows verified-at-seed; registerRateSources re-asserts sourceUrl onto governed rows
- [x] CURR-3: rate_verifications audit-trail table — every check recorded (status, observed values JSON, appliedAutomatically flag, evidence, method)
- [x] CURR-4: weekly Heartbeat extension — sweepRateSources in refreshReference cron (Mon 06:00 UTC): visible-text-only HTML fingerprinting (hardened against nonce/IP/markup churn — verified stable across double-fetch on all 4 HTML sources), PDF exact-hash, adjustor-cycle-aware cadence (PGA≤45d, GSC≤100d), staleness escalation to due
- [x] CURR-5: /api/scheduled/rateVerify endpoint — GET prioritized targets (change_detected > due > stale), POST findings with bounded auto-apply (≤8% volumetric adjustor-band delta auto-applies with audit row; larger flags for review, never silently mutates) + owner notification; cron-only auth verified live (403 unauthenticated on prod)
- [x] CURR-6: monthly AGENT cron created — "Monthly filed-rate verification (Meterly)", 3rd of month 07:30 UTC, taskUid rfvGk5hnNtjeaqn2ZoJ3ZZ persisted in platform_config; prompt: fetch queue → read official PDFs/pages → POST structured findings (covers SRP + Tucson Water which bot-block server fetches)
- [x] CURR-7: UI disclosure — Filed-rate currency panel on Tariffs page: per-source verified date, status chips (current/change_detected/due amber), source links
- [x] CURR-8: tests — rateCurrency.test.ts 15 specs (registry idempotency, target ordering, confirmed/auto-apply/flag/unreachable/source_moved paths, fingerprint change detection, staleness escalation); full suite 503 passed
- [x] CURR-9: deployed (checkpoint 8b74cc1f auto-published); AGENT cron live; sweep validated against prod DB twice — fixed 2 dead source URLs (APS, UNS Electric), browser-grade fetch headers, changed:[] stable rebaseline

## Nationwide systemic coverage (owner directive Jul 22: "should already be covering all nationwide — no per-utility asks")
- [x] NAT-1: Audit done — coverage was AZ/KY filed + state-representative imputed elsewhere; chose OpenEI URDB bulk CSV (public domain, 12MB → distilled 2.5MB / 4,935 default rates) for national filed-quality electric; existing nationwide zip3 service_territories registry (DB-backed) already covers territory; EIA drift detector governs imputed rows
- [x] NAT-2: resolveTerritoryUnified — hand-curated catalog (highest precision) → nationwide zip3 registry → fail-open to statewide; wired into pipeline (both call sites) + tariffs router
- [x] NAT-3: URDB bulk import — 160 utility-specific rows across 47 states / 83 major utilities (residential+commercial defaults, all-in standard-offer preferred over delivery-only, tiered/TOU structures converted, provenance = urdb_bulk source + openei rate URL, dedup vs hand-filed rows); alias map for legal-name mismatches (Eversource→CL&P/NSTAR, Evergy→KCP&L, National Grid→Niagara Mohawk, NYSEG/PSE&G abbreviations, Appalachian/OG&E normalization)
- [x] NAT-4: Gas/water nationwide — imputed state-average coverage confirmed for all states via existing EIA-based rows + weekly drift detection; filed upgrades flow automatically through the acquisition queue (demand-driven by site adds, never prompts)
- [x] NAT-5: Auto-enrollment — pipeline enqueues utility×commodity into rate_acquisition_queue (migration 0020) when a site's utility has no utility-specific rows (fire-and-forget, never blocks analysis); monthly agent GET includes acquisitions[]; POST→applyAcquisition inserts agent_acquired rows with sanity bounds ($0.01–1.50/unit, ≤$500/mo fixed); URDB import self-enqueues its own gaps (8 queued: TVA composite, ComEd res, APCo res, …)
- [x] NAT-6: Rate-change timeline — tariffs.rateChangeTimeline procedure (material audit-trail events) + RateActivityCard on Dashboard (auto-hides when empty; auto-applied/change-detected/acquired badges)
- [x] NAT-7: Pending-rate-case tracking — 3 docket-watch sources (APS ACC E-01345A-25-0105, TEP ACC 25-0103, LG&E/KU KY PSC raterequest) registered with sourceKind=docket, swept weekly, pending-case badges in the Filed-rate currency panel; docket changes never mutate tariff rows
- [x] NAT-8: Tests — rateCurrency.test.ts now 21 specs incl. applyAcquisition ×4 (insert/sanity-reject/failed/unknown-queue) + docket ×2 (governs contract, empty-governs sweep); full suite 509 passed | 8 skipped
- [x] NAT-9: Live validation — sweep run twice against prod DB (12 sources: 9 tariff + 3 docket, docket fingerprints stable, LG&E transient failures cleared, stale swgas flag reset); urdbDistilled.json lazy-loaded with prod-safe path fallback; Tariffs + Dashboard screenshots verified

## Next-step directives (owner Jul 22: "consider the optimal approach and do accordingly")
- [x] IMP-1: Per-site dollar-impact estimates — rateImpact.ts computeRateChangeImpact: signed volumetric/fixed deltas captured in applyAgentFinding, affected sites matched by utility×commodity×state, annual usage ladder (metered intervals ≥28d → bills ≥28d → honest null), impact json persisted on rate_verifications (migration 0021); hooked into BOTH auto-apply and flagged branches (flagged = "projected if adopted")
- [x] IMP-2: Impact surfaced — rateChangeTimeline payload carries impact; Dashboard RateActivityCard shows affected-site count + ±$X/yr chip (rose/emerald, per-site names, disclosure tooltip); rateVerifyHandler notification digest includes impact totals
- [x] DKT-1: Territory-driven docket auto-registration — stateDockets.ts: STATE_COMMISSION_DOCKETS registry (50 states + DC official commission docket-index URLs); ensureDocketCoverage() registers docket-state-xx watches only for states with actual sites (AZ + KY live); AZ URL fixed to the working ACC eDocket portal
- [x] DKT-2: Wired into the weekly sweep (best-effort, never blocks fingerprinting) — a site added in a new state picks up its commission watch on the next sweep, zero prompting
- [x] GWD-1: Gas/water auto-enqueue verified — pipeline auto-enrollment passes meter.commodity (gas/water sites enqueue like electric); covered by test
